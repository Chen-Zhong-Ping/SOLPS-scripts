...
class TaskQueue():
    ...
    def find_recently_completed(self):
        ...
        return None
    def __repr__(self):
        return "completed: "+str( sorted( [ t.taskid for t in self.completed ] ) )+";"+\
               "\nqueued: " +str( sorted( [ t.taskid for t in self.queue] ) )+";"+\
               "\nrunning: "+str( sorted( [ t.taskid for t in self.running ] ) )+"."
    def savestate(self):
        state = ""
        state += "queued\n"
        for t in self.queue:
...
""" % ( len(self.completed), max( times ), sum( times )/len(self.completed) )
        return message
...
class LauncherJob():
    ...
    def __init__(self,**kwargs):
        ...
        self.completed = 0; self.tock = 0; self.barriertest = None
